...
-------------------------------------------------------------------------------
    ...
    LG       부산동래구  051-9999-9999  이새롬      대리  lsr@na.. 010-... 
                                    :
                                    :
--------------------------------------------------------------------------------
*/
...
가정) 서울 여의도 LG 라는 회사에 근무하는 거래처 직원 명단이
      ...
      --> 1 개 행을 하드디스크상에서 읽어다가 메모리에 로드시켜주어야한다.
           즉 1* 40Byte를 하드디스크상에서 읽어다가 메모리에 로드시켜주어야한다.
...
SELECT 거래처직원명, 직급             | SELEC 거래처직원명, 직급
FROM 직원                             | FROM 거래처직원
...
--> 역정규화를 할까말까 판단할땐 업무 분석이 명확하게 되있어야 한다.
    보통 역정규화하는 테이블들의 데이터크기는 비슷하다.
...
테이블명 : 사원 → 부모테이블                                  테이블명 : 직급
----------------------------------------------------------     ---------------------
사원번호 사원명  주민번호   입사일       급여   직급코드          1 사원 ~ 10 회장
++++++++
----------------------------------------------------------
...
테이블명 : 사원가족 → 자식테이블             테이블 : 가족관계
-------------------------------------         ----------------------
주민번호  사원번호  관계    성명              관계내용     관계코드
++++++++  ========                              배우자         1
-------------------------------------           부모           2
9XXXXX...   7369    1    아이유                 자식           3
9XXXXX...   7370    1    강동원
21XXXX...   7370    3    강주혁
9XXXXX...   7371    1    이제훈
              :
...
1. 관계(relationship, relation)
   ...
   - 각 열(column)은 유일한 이름을 가지며 순서는 뭐의미하다.
...
5. 릴레이션(relation)
   ...
   - 중복된 튜플 (tuple = entity = 엔티티)을 포함하지 않는다.
     → 모두 상이함(튜플의 유일성)
   - 릴레이션 = 튜플(엔티티 = entuty)의 집합. 따라서 튜플의 순서는 무의미하다.
...
INSERT INTO TBL_TEST1(COL1, COL2) VALUES(NULL, 'STDUY'); --> 에러 발생
INSERT INTO TBL_TEST1(COL2) VALUES('STDUY');             --> 에러 발생
...
INSERT INTO TBL_TEST2(COL1, COL2) VALUES(NULL, 'STDUY'); --> 에러 발생
INSERT INTO TBL_TEST2(COL2) VALUES('STDUY');             --> 에러 발생
...
INSERT INTO TBL_TEST3(COL1, COL2) VALUES(NULL, 'STDUY'); --> 에러 발생
INSERT INTO TBL_TEST3(COL2) VALUES('STDUY');             --> 에러 발생
