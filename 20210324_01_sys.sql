...
-- 211.238.142.166 (강의장 내자리 아이피번호)

-- 오라클은 한번 실행된거(성공) 또 실행하면 에러남!
-- 제대로 구성해서 한번만 실행할 것
...
--> kab 라는 사용자를 만들겠다. (생성하겠다.)
--  패스워드는 java006$ 로 하겠다.
--  이 계정을 통해 생성하는 오라클 객체는(세그먼트들은)
--  기본적으로(default) TBS_EDA 라는 테이블 스페이스 생성할 수 있도록 설정하겠다.

--※ 생성된 오라클 사용자 계정(kab)을 통해
--   접속을 시도해 보았으나 접속 불가.
--   → 『CREATE SESSION』 권한이 없기 때문에...

--○ 생성된 오라클 사용자 계정(kab)에 
--   서버 접속이 가능할 수 있도록 CREATE SESSION 권한 부여 → SYS 가...
GRANT CREATE SESSION TO kab;
--==>> Grant을(를) 성공했습니다.


--○ 생성된 오라클 사용자 계정(KAB)의 
...
--○ 생성된 오라클 사용자 계정(KAB)에
--   테이블스페이스(TABLE_EDUA)에서 사용할 수 있는 공간(할당량)
--   의 크기를 무제한으로 지정.
ALTER USER KAB
